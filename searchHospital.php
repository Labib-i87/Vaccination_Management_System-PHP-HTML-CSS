...
$dhid = $_GET['dhid'];
if (!empty($dhid)){
  $query = mysqli_query($conn, "DELETE FROM hospital WHERE h_id = '$dhid'");
  if($query) {
    ...
    echo "<script>alert('ERROR: FAILED TO DELETE HOSPITAL')</script>";
    ?>
      <script>
        window.location.href = "searchHospital.php"
      </script>
...
if(count($_POST)>0) {
  if(!empty($_POST['name'])) {
    $name = $_POST['name'];
    $result = mysqli_query($conn, "SELECT * FROM hospital WHERE name LIKE '%".$name."%'");
  }
  else {
...
<div class=container>
<a style="float: right;" class="btn   btn-warning btn-lg py-3 px-5 mb-2 me-2" href="addHospital.php"
          role="button" aria-controls="#picker-editor">Add Hospital</a>
    <table class="table">
        ...
        <tbody>
            <tr style="vertical-align: middle;">
                ...
                <td class="text-center">
                    <a href='<?php echo "editHospital.php?dhid=$row[0]"?>'><span class="btn btn-warning">EDIT</span></a>
                </td>
                <td class="text-center">
                    <a href='<?php echo "searchHospital.php?dhid=$row[0]"?>'><span class="btn btn-danger">DELETE</span></a>
                </td>
