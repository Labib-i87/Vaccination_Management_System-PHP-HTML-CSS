            ...
            <section draggable="false" class="container pt-5" data-v-271253ee="">
                <section class="mb-10">
                    ...
                    <div class="card mx-4 mx-md-5 text-left shadow-5-strong"
                        ...
                        <form method="post" action="addVaccine.php" style="width: 23rem;">

                            <h3 class="fw-normal mb-3 pb-3" style="letter-spacing: 1px;">Add Vaccine</h3>

                            <div class="form-outline mb-4">
                              <input type="text" name="vname" id="text" class="form-control form-control-lg" />
                              <label class="form-label" for="form2Example18">Vaccine Name</label>
                            </div>

                            <div class="form-outline mb-4">
                              <input type="text" name="vprice" id="text" class="form-control form-control-lg" />
                              <label class="form-label" for="form2Example28">Price</label>
                            </div>
                            ...
                            <?php
                              if (isset($_POST['submit']) && isset($_POST['vname']) && isset($_POST['vprice']) 
                                  && !empty($_POST['vname']) && !empty($_POST['vprice']) ) {
                                $vname = $_POST['vname'];
                                $vprice = $_POST['vprice'];

                                $checker = mysqli_query($conn, "SELECT * FROM vaccine_price WHERE vac_name='$vname'");
                                if(mysqli_num_rows($checker) == 0) {
                                  $sql = "INSERT INTO vaccine_price(vac_name, price) values ('$vname', '$vprice')";
                                  $query = mysqli_query($conn, $sql);
                                  echo "<script>alert('VACCINE ADDED')</script>";
                                }
                                else{
                                  echo "<script>alert('ERROR: VACCINE ALREADY EXISTS')</script>";
                                }
                              }
