      ...
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav ms-auto">
            ...
            <button onclick="location.href='addInventoryH.php'" type="button" class="btn btn-dark ms-3"><- Go Back</button>
          </ul>
        </div>
      </div>
    </nav>
    ...
    <form method="post" action='<?php echo "addInventoryV.php?hid=$hid"?>'>
        <div class="inner-form">
            <div class="input-field first-wrap">
                ...
                <input type="text" name="vname" id="search" placeholder="Enter Vaccine Name" />
            </div>
            
            <div class="input-field second-wrap">
                <button type="submit" name="save" class="btn-search">
                    SEARCH
                </button>
...
if(count($_POST)>0) {
    if(!empty($_POST['vname'])) {
      $vname = $_POST['vname'];
      $result = mysqli_query($conn, "SELECT * FROM vaccine_price where vac_name LIKE '%".$vname."%'");
    }
    else {
        ...
        <tbody>
            <tr style="vertical-align: middle;">
                ...
                <td class="text-center">
                    <a href='<?php echo "addInventoryQ.php?vname=$row[0]&hid=$hid"?>'><span class="btn btn-warning">Select</span></a>
                </td>
            </tr>
